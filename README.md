...
### 📋 1. Bảng Tra Cứu 44 Âm IPA (`ipa.html`)
* **2 Chế độ hiển thị:**
...
```text
ipa/
├── ipa.html                 # Trang tra cứu bảng 44 âm IPA
...
│   ├── app-lookup.js        # Logic xử lý giao diện tra cứu (ipa.html)
...
1. **Chạy trực tiếp (Không cần Node.js hay Build Tool):**
   ...
   * Mở trực tiếp file `ipa.html` hoặc `exercise.html` bằng bất kỳ trình duyệt web hiện đại nào (Google Chrome, Microsoft Edge, Apple Safari, Mozilla Firefox).

2. **Cách luyện tập hiệu quả:**
   * Mở `ipa.html` để học qua cách đọc khẩu hình và nghe mẫu 44 âm.
